...
namespace LN_WEB.Models
{
    public class UsuarioModel
    {
        ...
        public int RemoverUsuario(long q)
        {
            ...
            {
                string token = HttpContext.Current.Session["Token"].ToString();
                string url = ConfigurationManager.AppSettings["urlApi"].ToString() + "api/RemoverUsuario?q=" + q;
        ...
        public void UpdatePathUsuario(UsuarioEnt entidad)
        {
            using (var client = new HttpClient())
            {
                ...
                string url = ConfigurationManager.AppSettings["urlApi"].ToString() + "api/UpdatePathUsuario";
                JsonContent body = JsonContent.Create(entidad); //Serializar
